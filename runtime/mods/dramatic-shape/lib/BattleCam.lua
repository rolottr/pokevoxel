...
-- Hold the rig perfectly still (an excluded headset mode used this while a session ran). The
...
-- picture that sways on its own in a headset reads as the world lurching --
...
-- or the rig's own alone whenever both are being withheld (a fixed tracked
...
function BattleCam.rig(arena, groundY, canonical)
  ...
  -- A tracked view asks for the same stillness for its own reason (see BattleCam.still)
