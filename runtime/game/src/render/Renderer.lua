...
function Renderer:endFrame(zones, worldZones)
  ...
  if self.worldOverride then
    ...
    love.graphics.setColor(1, 1, 1, 1)
    love.graphics.setScissor(0, 0, ww, wh)
    local worldBlend, worldAlpha = love.graphics.getBlendMode()
    -- A world override is a complete window-sized frame. Voxel water may use
    -- its otherwise-unused alpha channel as a readable camera-depth texture;
    -- replace preserves the RGB frame without treating that data as opacity.
    love.graphics.setBlendMode("replace", "premultiplied")
    local overrideW, overrideH = self.worldOverride:getDimensions()
    local overrideSx, overrideSy = ww / overrideW, wh / overrideH
    local loveMajor = love.getVersion()
    if love.system and love.system.getOS and love.system.getOS() == "iOS" and loveMajor >= 12 then
      love.graphics.draw(self.worldOverride, 0, wh, 0, overrideSx, -overrideSy)
    else
      love.graphics.draw(self.worldOverride, 0, 0, 0, overrideSx, overrideSy)
    end
    love.graphics.setBlendMode(worldBlend, worldAlpha)
    love.graphics.setScissor()
